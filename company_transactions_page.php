...
<body>
  ...
  <div id="searchLoadingOverlay" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(255, 255, 255, 0.95); z-index: 9999; align-items: center; justify-content: center; flex-direction: column;">
    <div style="text-align: center;">
      <div style="border: 8px solid #f3f3f3; border-top: 8px solid #007bff; border-radius: 50%; width: 60px; height: 60px; animation: spin 1s linear infinite; margin: 0 auto 20px;"></div>
      <h4 style="color: #333; margin: 0;">Loading Results...</h4>
      <p style="color: #666; margin-top: 10px;">Please wait while we fetch the data</p>
    </div>
  </div>
  <style>
    @keyframes spin {
  ...
  <div class="box shadow">
    ...
    <form action="" onsubmit="showCustomer(event); return false;">
    ...
    <div class="tab-content mt-3" id="transactionTabsContent">
      <div class="tab-pane fade show active" id="arriving" role="tabpanel">
        ...
        <div class="box shadow mt-3">
          <h6>Top 10 Products Arriving Based on User Filters</h6>
      ...
      <div class="tab-pane fade" id="leaving" role="tabpanel">
        ...
        <div class="box shadow mt-3">
          <h6>Top 10 Products Leaving Based on User Filters</h6>
  ...
  <script>
    // Cache for continent list
    let continentList = null;
    let continentListPromise = null;
    
    // Function to get continent list
    function getContinentList() {
      if (continentList !== null) {
        return Promise.resolve(continentList);
      }
      if (continentListPromise === null) {
        continentListPromise = fetch('company_transactions_output.php?continent=&date1=2019-01-01&date2=2019-01-02')
          .then(response => response.json())
          .then(data => {
            console.log('API response:', data);
            if (data["Continent Name List All: "]) {
              continentList = [...new Set(data["Continent Name List All: "])].sort();
              console.log('Loaded continents:', continentList);
            } else {
              continentList = [];
            }
      ...
      return continentListPromise;
    }
    
    // Load default search on page load
    window.addEventListener('load', function() {
      // Set default search type to Country
      document.getElementById("searchType").value = "country";
      
      // Trigger the field update to show the country input
      updateSearchFields();
      ...
      // Wait for country field to be created, then set value and trigger search
      setTimeout(() => {
    ...
    function updateSearchFields() {
      ...
      switch(searchType) {
        ...
        case 'continent':
          ...
          html = '<select name="continent" id="continent" required><option value="">Loading continents...</option></select>';
          searchFieldsDiv.innerHTML = html;
          
          // Load continents and update dropdown
          getContinentList().then(list => {
            ...
            if (list && list.length > 0) {
              ...
              options += '<option value="">No continents found</option>';
            }
            document.getElementById('continent').innerHTML = options;
          });
          return; // Early return since we handle the innerHTML directly
        case 'country':
    ...
    function showCustomer(event) {
      ...
      const url = "company_transactions_output.php?" + new URLSearchParams(inputs).toString();
      console.log(url);
      ...
      xhttp.onload = function() {
        ...
        if(this.readyState === 4 && this.status === 200) {
          ...
          document.getElementById("arrivingTotalDisplay").textContent = "There might not be any arriving transactions.";
          document.getElementById("arrivingTable").innerHTML = '<p style="text-align: center; padding: 20px; color: #6c757d;">There might not be any arriving transactions.</p>';
          document.getElementById("arrivingChart").innerHTML = '';
          document.getElementById("leavingTotalDisplay").textContent = "There might not be any leaving transactions.";
          document.getElementById("leavingTable").innerHTML = '<p style="text-align: center; padding: 20px; color: #6c757d;">There might not be any leaving transactions.</p>';
          document.getElementById("leavingChart").innerHTML = '';
          document.getElementById("adjustmentsTotalDisplay").textContent = "There might not be any adjustments.";
          document.getElementById("adjustmentsTable").innerHTML = '<p style="text-align: center; padding: 20px; color: #6c757d;">There might not be any adjustments.</p>';
          ...
          // Validate city name if it was provided
          if (inputs["city"]) {
            const cityListAll = temp["City Name List All: "];
            const enteredCity = inputs["city"].trim();
            
            if (cityListAll && cityListAll.length > 0 && !cityListAll.includes(enteredCity)) {
              errorMessageDiv.textContent = "City '" + enteredCity + "' not found in database. Please check the spelling and try again.";
              errorMessageDiv.style.display = "block";
              return;
            }
          }
          ...
          // Create arriving chart using aggregated data from PHP
          if(temp["Product Name Arriving"] && temp["Product Name Arriving"].length > 0) {
            const arrivingProducts = temp["Product Name Arriving"].slice(0, 10);
            const arrivingQuantities = temp["Total Quantity Shipped Arriving"].slice(0, 10);
            
            createBarChart("arrivingChart", {
              labels: arrivingProducts,
              values: arrivingQuantities
            }, {
              xAxisTitle: "Product",
          ...
          // Create leaving chart using aggregated data from PHP
          if(temp["Product Name Leaving"] && temp["Product Name Leaving"].length > 0) {
            const leavingProducts = temp["Product Name Leaving"].slice(0, 10);
            const leavingQuantities = temp["Total Quantity Shipped Leaving"].slice(0, 10);
            
            createBarChart("leavingChart", {
              labels: leavingProducts,
              values: leavingQuantities
            }, {
              xAxisTitle: "Product",
              yAxisTitle: "Total Quantity"
            });
