...
<script>
 $(document).ready(function() { 
    ...
    if (activeTab) { 
        $('#companyTypeTabs a[href="' + activeTab + '"]').tab('show');  
    } 
    $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
        localStorage.setItem('activeCompanyTab', $(e.target).attr('href'));  
    }); 
});
...
document.getElementById('manufacturerForm').addEventListener('submit', function(e){
    ...
    Object.keys(fieldMap).forEach(id => {
        ...
        data[fieldMap[id]] = val;
    });
    
    fetch('updates_output.php?' + new URLSearchParams(data))
        .then(res=>{
            if (!res.ok) {
        ...
        .then(text=>{
            try {
                ...
                // Auto-hide success message and reload page after 10 seconds
                setTimeout(() => {
                    resultDiv.style.display = 'none';
                    localStorage.setItem('activeCompanyTab', '#manufacturer');
                    location.reload();
                }, 1500);
            } catch(e) {
...
document.getElementById('distributorForm').addEventListener('submit', function(e){
    ...
    if (fromOrig || toOrig || fromCompany || toCompany) {
        // If one original is provided, both originals must be provided
        if ((fromOrig || toOrig) && !(fromOrig && toOrig)) {
            errorDiv.textContent = 'You must provide both original "From" and "To" company names.';
            errorDiv.style.display = 'block';
            return;
        }
        
        // If originals are provided, at least one new company must be provided
        if (fromOrig && toOrig && !fromCompany && !toCompany) {
            errorDiv.textContent = 'You must provide at least one new company name (From or To) when updating logistics.';
            errorDiv.style.display = 'block';
            return;
        }
        
        // Validate companies exist in database
        if (fromOrig && companyList.length > 0 && !companyList.includes(fromOrig)) {
            errorDiv.textContent = 'Original "From" company "' + fromOrig + '" not found in database.';
            errorDiv.style.display = 'block';
            return;
        }
        
        if (fromCompany && companyList.length > 0 && !companyList.includes(fromCompany)) {
            errorDiv.textContent = 'New "From" company "' + fromCompany + '" not found in database.';
            errorDiv.style.display = 'block';
            return;
        }
        
        if (toOrig && companyList.length > 0 && !companyList.includes(toOrig)) {
            errorDiv.textContent = 'Original "To" company "' + toOrig + '" not found in database.';
            errorDiv.style.display = 'block';
            return;
        }
        
        if (toCompany && companyList.length > 0 && !companyList.includes(toCompany)) {
            errorDiv.textContent = 'New "To" company "' + toCompany + '" not found in database.';
            errorDiv.style.display = 'block';
            return;
        }
        ...
        if (fromCompany && toCompany && fromCompany === toCompany) {
            errorDiv.textContent = 'New "From" company and new "To" company cannot be the same.';
            errorDiv.style.display = 'block';
            return;
        }
        ...
        if (fromCompany && fromCompany === currentName) {
            errorDiv.textContent = 'New "From" company cannot be the same as the current company being updated.';
            errorDiv.style.display = 'block';
            return;
        }
        
        if (toCompany && toCompany === currentName) {
            errorDiv.textContent = 'New "To" company cannot be the same as the current company being updated.';
            errorDiv.style.display = 'block';
            return;
        }
        
        if (fromOrig && fromOrig === currentName) {
            errorDiv.textContent = 'Original "From" company cannot be the same as the current company being updated.';
            errorDiv.style.display = 'block';
            return;
        }
        
        if (toOrig && toOrig === currentName) {
            errorDiv.textContent = 'Original "To" company cannot be the same as the current company being updated.';
            errorDiv.style.display = 'block';
            return;
        }
    }
    ...
    fetch('updates_output.php?' + new URLSearchParams(data))
    .then(text=>{
        // Always show success if HTTP status is 200, even if server returns no JSON
        resultDiv.className = 'success';
        ...
        setTimeout(() => {
            resultDiv.style.display = 'none';
            localStorage.setItem('activeCompanyTab', '#distributor');
            location.reload();
        }, 1500);
    })
...
document.getElementById('retailerForm').addEventListener('submit', function(e){
    ...
    if (newName !== '') {
        if (companyList.length > 0 && companyList.includes(newName)) {
            errorDiv.textContent = 'Company "' + newName + '" already exists in database. Please choose a different name.';
            errorDiv.style.display = 'block';
            return;
        }
    ...
    fetch('updates_output.php?' + new URLSearchParams(data))
        ...
        .then(text=>{
            try {
                ...
                setTimeout(() => {
                    resultDiv.style.display = 'none';
                    localStorage.setItem('activeCompanyTab', '#retailer');
                    location.reload();
                }, 1500);
            } catch(e) {
                console.error('Invalid JSON response:', text);
