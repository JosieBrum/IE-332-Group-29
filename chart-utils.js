...
function createColorfulPieChart(elementId, data, options = {}) {
  ...
  const chartOptions = {
    ...
    legend: {
      show: options.showLegend !== false,
      position: options.legendPosition || 'bottom',
      height: options.legendHeight || undefined,
      itemMargin: {
        horizontal: 5,
        vertical: 2
      },
...
function createHistogram(elementId, data, title, xAxisTitle, yAxisTitle) {
  console.log(`createHistogram called for ${elementId} with title: ${title}, xAxis: ${xAxisTitle}, yAxis: ${yAxisTitle}`);
  ...
  console.log(`Data length: ${data.length}, first 5 values:`, data.slice(0, 5));
  
  // Convert to numbers and sort
  const values = data.map(v => Number(v)).filter(v => !isNaN(v)).sort((a, b) => a - b);
  
  console.log(`After conversion: ${values.length} valid values, first 5:`, values.slice(0, 5));
  ...
  console.log('Creating histogram with layout:', layout);
...
function createRRCMap(elementId, rrcData, countryToISO3, continentCountries) {
  ...
  rrcData.forEach(item => {
    if (item.country) {
      ...
      if (iso) {
        locations.push(iso);
        const rrcValue = parseFloat(item["rrc value"]);
        values.push(rrcValue);
        hoverTexts.push(`${item.country}: ${rrcValue.toFixed(2)}%`);
      }
      ...
      countries.forEach(c => {
        locations.push(c);
        values.push(rrcValue);
        hoverTexts.push(`${item.continent}: ${rrcValue.toFixed(2)}%`);
      });
    }
  });
  ...
  const layout = {
    title: {
      text: 'Regional Risk Concentration Map',
      font: { size: 16 }
    },
