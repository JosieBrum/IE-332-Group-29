...
switch ($onornot){
    ...
    case 3:
        ...
        break;
    case 4: 
        $filiter = "AND ('".$city."' = '' OR l.ContinentName   = '".$city."') ";
        break;
    default: 
        $filiter = "AND ('".$company."' = '' OR c.CompanyName   = '".$company."') ";        
        break;
...
JOIN Company src ON src.CompanyID = s.DestinationCompanyID
...
WHERE t.Type = 'Shipping'
  ...
  AND ('Chicago' = '' OR lsrc.City = 'Chicago')
  AND ('' = '' OR lsrc.CountryName = '')
  AND ('' = '' OR lsrc.ContinentName = '')
...
JOIN InventoryTransaction t ON t.TransactionID = s.TransactionID
JOIN Product p ON p.ProductID = s.ProductID
JOIN Company dst ON dst.CompanyID = s.SourceCompanyID
...
$enteringID = array();
$enteringDate = array();
$enteringCompany = array();
$enteringProduct = array();
$enteringquantity = arry();
...
$output['AdjustmentID:'] = $adjustmentID;
...
$output['Adjustment Reason:'] = $adjustmentquantity;
...
$output['Entering ID:'] = $arrivingID;
$output['Entering Date:'] = $arrivingDate;
$output['Entering Company:'] = $arrivingCompany;
$output['Entering Product:'] = $arrivingProduct;
$output['Entering Quantity:'] = $arrivingquantity;

while ($row3 = $result3->fetch_assoc()) {
    $leavingID[] = $row2['ShipmentID'];
    $leavingDate[] = $row2['Date'];
    $leavingCompany[] = $row2['Company'];
    $leavingProduct[] = $row2['Product'];
    $leavingquantity[] = $row2['Quantity'];
}
$output['LeavingID:'] = $leavingID;
...
if (isset($city)) {
    while ($row0000 = $result0000->fetch_assoc()) {
        $citylistAll[] = $row0000['ContinentName'];
    }
